...
include"../config.php";
$suggestionP1=new suggestionC();
$listesuggestionClient=$suggestionP1->afficherSuggestionsClient($_SESSION['l']);
...
<div class="row">
    <?PHP
foreach($listesuggestionClient as $row){
	?>

  <div class="col-sm-6">
    <div class="card">
      <div class="card-body">
        <h1 class="card-title">Client:<?PHP echo $row['idUtilisateur']; ?></h1>
         <h3 class="card-title">localisation:<?PHP echo $row['localisation']; ?></h3>
        <p class="card-text">Service:<?PHP echo $row['Service']; ?></p>
         <p class="card-text">Note Service:<?PHP echo $row['noteService']; ?></p>
          <p class="card-text">Degré de satisfaction:<?PHP echo $row['serviceGlobal']; ?></p>
        <p>In- <?PHP echo $row['date']; ?></p>
	     <form method="POST" action="supprimerSuggestion.php" style="border: none">
              <input type="submit" name="supprimer" value="supprimer vos retour" class="btn btn-primary" >
	<input type="hidden" value="<?PHP echo $row['numSuggestion']; ?>" name="numSuggestion">
    <a href="modifierSuggestion.php?numSuggestion=<?PHP echo $row['numSuggestion']; ?>" class="btn btn-primary">Modifier vos retour</a>
	</form>
      </div>
    </div>
  </div>   
